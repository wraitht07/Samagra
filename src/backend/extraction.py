...
class ExtractedClause(BaseModel):
    ...
    grades_or_parameters: List[str] = Field(default_factory=list)
    certification_demands: List[str] = Field(default_factory=list)
...
IS_PATTERN = re.compile(
    r"\bIS\s*[:\s]?\s*([0-9]+(?:\s*\(Part\s*[0-9]+\))?(?:\s*:\s*[0-9]{4})?)",
    re.IGNORECASE,
)
